...
const Switch = () => {
  ...
  const toggleDarkMode = () => {
    setIsDarkMode(prevMode => {
      ...
      localStorage.setItem('darkMode', newMode); // Simpan status mode di localStorage

      // Mengubah kelas body sesuai dengan mode
      if (newMode) {
        document.body.classList.add('bg-gray-900', 'text-white');
      } else {
        document.body.classList.remove('bg-gray-900', 'text-white');
  ...
  return (
    <label className="inline-flex items-center relative">
      ...
      <svg height={512} width={512} viewBox="0 0 24 24" data-name="Layer 1" id="Layer_ 1" xmlns="http://www.w3.org/2000/svg" className="fill-black opacity-60 peer-checked:opacity-70 peer-checked:fill-white absolute w-6 h-6 right-[13px]">
